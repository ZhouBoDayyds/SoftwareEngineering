...
def repair_program_io(code):
    #正则表达式模式，用于匹配两种不同的输入输出格式
    pattern_case1_in = re.compile("In ?\[\d+]: ?")  # Jupyter Notebook输入格式
    pattern_case1_out = re.compile("Out ?\[\d+]: ?")  # Jupyter Notebook输出格式
    pattern_case1_cont = re.compile("( )+\.+: ?")  # Jupyter Notebook多行输入格式

    pattern_case2_in = re.compile(">>> ?")  # python交互式命令行输入格式
    pattern_case2_cont = re.compile("\.\.\. ?")  # python交互式命令行多行输入格式
...
def get_vars(ast_root):
    #使用集合推导式从AST中提取所有的变量名，并排除掉加载上下文中的变量名
    return sorted(
...
def PythonParser(code):
    ...
    tokenized_code = [] #初始化标记化代码列表

    def first_trial(_code):
        #尝试生成代码的第一个标记，以检查是否成功
        if len(_code) == 0:
            return True
        try:
    ...
    while not bool_finished:
        ...
        while not bool_success_next:
            try:
                ...
                # print("Failed line: ")
                # print sys.exc_info()
                #如果在获取标记时发生异常，使用wordpunct_tokenize进行处理
