...
class SqlangParser():
    ...
    def parseStrings(self, tok):
        #如果tok是TokenList对象，遍历TokenList中的每个Token对象
    ...
    @staticmethod
    def getTokens(parse):
        #初始化一个空列表，用于存储平铺后的Token字符串
        flatParse = []
        #遍历解析对象列表
        for expr in parse:
            #对每个解析对象进行平铺，即将嵌套的TokenList展开成单个Token
    ...
    def removeWhitespaces(self, tok):
        #判断tok是否为TokenList对象
        if isinstance(tok, sqlparse.sql.TokenList):
            #初始化一个列表，用于存储非空白Token
            tmpChildren = []
            #便利TokenList中的每个Token对象
            for c in tok.tokens:
                if not c.is_whitespace:
                    tmpChildren.append(c)

            #将TokenList中的Token替换为不包含空白Token的新列表
            tok.tokens = tmpChildren
            #再次遍历TokenList，对每个Token递归地调用removeWhitespaces方法
            for c in tok.tokens:
                self.removeWhitespaces(c)

    def identifySubQueries(self, tokenList):
        #初始化标志变量，指示当前TokenList是否为子查询
        isSubQuery = False

        #遍历TokenList中的每个Token
